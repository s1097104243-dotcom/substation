...
APP_BACKUP = Path(__file__).with_name("app backup.py")
runpy.run_path(str(APP_BACKUP), run_name="__main__")
import io
import re
from pathlib import Path
...
def build_data_maps(cleaned_df: pd.DataFrame) -> dict:
    ...
    for plan_name, plan_df in plan_groups:
        ...
        all_data_maps[str(plan_name).strip()] = data_map
...
def build_result_excel(cleaned_df: pd.DataFrame, all_data_maps: dict) -> bytes:
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        summary_rows = [{"Plan": p, "筆數": len(m)} for p, m in all_data_maps.items()]
        ...
        for plan_name, data_map in all_data_maps.items():
            plan_df = pd.DataFrame([{"Key": k, "Actual": v} for k, v in data_map.items()])
            plan_df.to_excel(writer, sheet_name=unique_sheet_name(plan_name, used_names), index=False)
...
def process_sources_only(source_files):
    ...
    cleaned_df = final_df[[c for c in keep_cols if c in final_df.columns]].copy()
    ...
    return {
        "file_bytes": build_result_excel(cleaned_df, all_data_maps),
        "parse_logs": parse_logs,
        "write_logs": [f">>> [{p}]：輸出 {len(m)} 筆" for p, m in all_data_maps.items()],
...
def render_template_downloads() -> None:
    st.subheader("檔案下載")
    assets = [
        (Path("assets") / "公版_IED試驗報告.xlsm", "下載試驗報告", "application/vnd.ms-excel.sheet.macroEnabled.12"),
        (Path("assets") / "公版_變電所_測試程序.psx", "下載測試程序", "application/octet-stream"),
    ]
    for idx, (path, label, mime) in enumerate(assets):
        if path.exists():
            with path.open("rb") as f:
                st.download_button(
                    ...
                    file_name=path.name,
                    mime=mime,
                    key=f"asset_download_{idx}_{path.name}",
...
source_uploads = st.file_uploader("來源資料檔案（可多選）", type=["xlsx", "xlsm", "xls"], accept_multiple_files=True)
        ...
        with st.spinner("處理中，請稍候..."):
            try:
                ...
                st.download_button(
                    ...
                    key="result_for_import_download",
                    use_container_width=True,
                )
import io
import re
...
def build_result_excel(cleaned_df: pd.DataFrame, all_data_maps: dict) -> bytes:
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        summary_rows = [{"Plan": plan_name, "筆數": len(data_map)} for plan_name, data_map in all_data_maps.items()]
        pd.DataFrame(summary_rows).to_excel(writer, sheet_name="PlanSummary", index=False)
